# src/evaluate.py
"""
Threshold tuning using enrollment crops (aligned 112x112).
"""
...
def _is_aligned_crop(img, req):
    h, w = img.shape[:2]
    return (w, h) == (int(req[0]), int(req[1]))


def load_embeddings_for_person(embedder, person_dir, cfg):
...
def pairwise_distances(embs_a, embs_b, same: bool):
    dists: List[float] = []
...
def sweep_thresholds(genuine, impostor, cfg: EvalConfig):
